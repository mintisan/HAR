...
def evaluate_model(trainX, trainy, testX, testy):
    ...
    model.compile(loss='categorical_crossentropy', optimizer=adam, metrics=['accuracy']) # default lr=0.01
...
def run_experiment(data_dir=""):
    ...
    plot_model(model, to_file='./keras_cnn_har_model.png', show_shapes=True)  # model structure
    model.save('keras_cnn_har_model.h5')  # model
    netron.start('keras_cnn_har_model.h5')  # model structure
    ...
    plt.xlabel('epoch')
    plt.savefig("keras_cnn_har_model_epoch" + '.png')
    # plt.show()
    plt.close()
    plt.clf()
